...
class Test(unittest.TestCase):
    ...
    def test_bitarray_set_bounds_upper(self):
        ...
        self.assertEqual(bitmap.tolist(), torch.ByteTensor([0x0, 0x80]).tolist())
    ...
    def test_bitarray_set(self):
        ...
        bitmap = bitarray.set(bitmap, 1, True)
        self.assertEqual(bitmap, torch.ByteTensor([0x3]))
        bitmap = bitarray.set(bitmap, 1, False)
        self.assertEqual(bitmap, torch.ByteTensor([0x1]))

    def test_bitarray_get(self):
        bitmap = torch.ByteTensor([5])
        self.assertEqual(torch.BoolTensor([True]), bitarray.get(bitmap, 0))
        self.assertEqual(torch.BoolTensor([False]), bitarray.get(bitmap, 1))
        self.assertEqual(torch.BoolTensor([True]), bitarray.get(bitmap, 2))
